...
function getQrisInstances() {
  ...
  if (!qrisGen || !qrisPaymentChecker) {
    const cfg = {
      storeName: NAMA_STORE || 'BotVPN',
      auth_username: ORDERKUOTA_AUTH_USERNAME,
      auth_token: ORDERKUOTA_AUTH_TOKEN,
      baseQrString: ORDERKUOTA_BASE_QR,
      // logoPath opsional (kalau punya logo PNG)
      // logoPath: path.join(__dirname, 'logo-qris.png'),
    };

    // theme1 = biru (default), bisa ganti 'theme2' kalau mau meta style
    qrisGen = new QRISGenerator(cfg, 'theme1');
    qrisPaymentChecker = new PaymentChecker({
      auth_token: cfg.auth_token,
      auth_username: cfg.auth_username,
    });
  }

  return { qrisGen, qrisPaymentChecker };
}
...
db.run(`
  CREATE TABLE IF NOT EXISTS qris_payments (
    ...
    amount INTEGER NOT NULL,       -- nominal yang DIBAYAR (base + unik)
    base_amount INTEGER DEFAULT 0, -- nominal yang diminta user
    unique_suffix INTEGER DEFAULT 0, -- 3 digit acak
    status TEXT NOT NULL,          -- 'pending','paid','expired','failed'
    created_at INTEGER NOT NULL,
    paid_at INTEGER
  )
`);
// Pastikan kolom base_amount & unique_suffix ada (untuk DB lama)
db.run(`ALTER TABLE qris_payments ADD COLUMN base_amount INTEGER DEFAULT 0`, (err) => {
  if (err && !String(err.message).includes('duplicate column')) {
    logger.error('⚠️ Gagal ALTER TABLE tambah base_amount:', err.message);
  }
});

db.run(`ALTER TABLE qris_payments ADD COLUMN unique_suffix INTEGER DEFAULT 0`, (err) => {
  if (err && !String(err.message).includes('duplicate column')) {
    logger.error('⚠️ Gagal ALTER TABLE tambah unique_suffix:', err.message);
  }
});
// ======================= END SECTION: PAYMENT - DATABASE TABLES =============

// ============================================================================
// SECTION: PAYMENT - WORKER AUTO-TOPUP QRIS (TABEL qris_payments)
// - startQrisAutoTopupChecker : loop cek invoice QRIS auto (OrderKuota)
//   • Set expired kalau lewat timeout
//   • Set paid + tambahin saldo user kalau status PAID
// ============================================================================
function startQrisAutoTopupChecker() {
  const INTERVAL_MS =
    QRIS_CHECK_INTERVAL_MS && QRIS_CHECK_INTERVAL_MS >= 1000
      ? QRIS_CHECK_INTERVAL_MS
      : 5000;

  const EXPIRE_AFTER_MIN =
    QRIS_PAYMENT_TIMEOUT_MIN && QRIS_PAYMENT_TIMEOUT_MIN > 0
      ? QRIS_PAYMENT_TIMEOUT_MIN
      : 15;

  setInterval(() => {
    const now = Date.now();
    const expireThreshold = now - EXPIRE_AFTER_MIN * 60 * 1000;

    db.all(
      'SELECT * FROM qris_payments WHERE status = ?',
      ['pending'],
      async (err, rows) => {
        if (err) {
          logger.error('❌ Gagal ambil qris_payments pending:', err.message);
          return;
        }
        ...
        for (const row of rows) {
          try {
            // 1) Kalau sudah terlalu lama → expired
            if (row.created_at < expireThreshold) {
              db.run(
                'UPDATE qris_payments SET status = ? WHERE id = ? AND status = ?',
                ['expired', row.id, 'pending'],
                function (errExp) {
                  if (errExp) {
                    logger.error(
                      `❌ Gagal set qris_payments expired (id=${row.id}):`,
                      errExp.message
                    );
                    return;
                  }
                  ...
                  if (this.changes === 0) {
                    // kemungkinan sudah di-expired manual / lewat /cekqris
                    return;
                  }

                  logger.info(
                    `⏰ QRIS invoice ${row.invoice_id} di-set EXPIRED (timeout checker)`
                  );
                  ...
                  const msgUser =
                    '⏰ <b>Topup QRIS Kadaluarsa</b>\n\n' +
                    `🧾 Invoice : <code>${row.invoice_id}</code>\n` +
                    `💳 Nominal : <b>Rp${row.amount.toLocaleString('id-ID')}</b>\n\n` +
                    `QRIS ini sudah melewati batas waktu <b>${timeoutMin} menit</b> dan tidak dapat dipakai lagi.\n\n` +
                    'Silakan buat topup baru jika masih ingin mengisi saldo.';

                  bot.telegram
                    .sendMessage(chatId, msgUser, { parse_mode: 'HTML' })
                    .catch((e) => {
                      logger.error(
                        `❌ Gagal kirim notif expired QRIS ke user (invoice=${row.invoice_id}):`,
                        e
                      );
                    });
                }
            ...
            // 2) Cek ke API OrderKuota (pakai fungsi yang sama dengan /cekqris)
            const statusInfo = await checkQrisInvoiceStatus(
              row.invoice_id,
              row.amount
            );

            // Normalisasi status ke huruf besar
            const status =
              statusInfo && statusInfo.status
                ? String(statusInfo.status).toUpperCase()
                : 'PENDING';

            // == 1) Kalau PENDING -> lanjut ke invoice berikutnya
            if (status === 'PENDING') {
              continue;
            }

            // == 2) Kalau EXPIRED dari API -> tandai expired + kirim notif
            if (status === 'EXPIRED') {
              db.run(
                'UPDATE qris_payments SET status = ? WHERE id = ? AND status = ?',
                ['expired', row.id, 'pending'],
                function (errExp) {
                  if (errExp) {
                    logger.error(
                      `❌ Gagal set qris_payments expired (API) (id=${row.id}):`,
                      errExp.message
                    );
                    return;
                  }

                  if (this.changes === 0) return; // sudah bukan pending

                  logger.info(
                    `⛔ QRIS invoice ${row.invoice_id} di-set EXPIRED (status API)`
                  );
                  ...
                  const msgUser =
                    ...
                    `🧾 Invoice : <code>${row.invoice_id}</code>\n` +
                    `💳 Nominal : <b>Rp${row.amount.toLocaleString('id-ID')}</b>\n\n` +
                    'Status pembayaran dari penyedia QRIS menunjukkan bahwa transaksi ini <b>EXPIRED</b> dan tidak bisa diproses lagi.\n\n' +
                    'Silakan buat topup baru jika masih ingin mengisi saldo.';
                  ...
                  bot.telegram
                    ...
                    .catch((e) => {
                      logger.error(
                        `❌ Gagal kirim notif expired (API) ke user (invoice=${row.invoice_id}):`,
                        e
                      );
                    });
                }
            ...
            // == 3) Kalau bukan PAID juga (status aneh) -> anggap pending
            if (status !== 'PAID') {
              continue;
            }

            // === Sampai sini berarti status = PAID ===
            const paidAt = statusInfo.paid_at || now;

            // 3) Update DB jadi paid (kalau masih pending)
            db.run(
              'UPDATE qris_payments SET status = ?, paid_at = ? WHERE id = ? AND status = ?',
              ['paid', paidAt, row.id, 'pending'],
              function (errUpdate) {
                if (errUpdate) {
                  logger.error(
                    '❌ Gagal set qris_payments ke paid:',
                    errUpdate.message
                  );
                  return;
                }
                if (this.changes === 0) return; // sudah pernah diproses

                // 4) Hitung bonus & tambah saldo user
const { bonus, percent } = calculateTopupBonus(row.amount);
const totalCredit = row.amount + bonus;

db.run(
  'UPDATE users SET saldo = saldo + ? WHERE user_id = ?',
  [totalCredit, row.user_id],
  (errSaldo) => {
    if (errSaldo) {
      logger.error(
        '❌ Gagal menambah saldo user dari QRIS:',
        errSaldo.message
      );
      return;
    }

    const baseRefId = `qris_auto_${row.invoice_id}`;
    const refId =
      bonus > 0
        ? `${baseRefId}_bonus${bonus}_p${percent}`
        : baseRefId;
    ...
    db.run(
      ...
      [
        row.user_id,
        totalCredit,              // catat total saldo yang masuk
        'qris_auto_topup',
        refId,
        now,
      ],
      (errTrx) => {
        if (errTrx) {
          logger.error(
            '⚠️ Gagal mencatat transaksi qris_auto_topup:',
            errTrx.message
          );
        }
      }
    ...
    // 5) Kirim notif ke user + (opsional) grup
    db.get(
      'SELECT saldo FROM users WHERE user_id = ?',
      [row.user_id],
      async (errUser, userRow) => {
        if (errUser) {
          logger.error(
            '❌ Gagal ambil saldo user setelah topup QRIS:',
            errUser.message
          );
          return;
        }

        const saldoNow = userRow ? userRow.saldo : 0;
        const chatId = row.user_id;
        ...
        let msgUser =
          ...
          `🧾 Invoice : <code>${row.invoice_id}</code>\n` +
          `💰 Nominal bayar : <b>Rp${row.amount.toLocaleString('id-ID')}</b>\n`;

        if (bonus > 0) {
          msgUser +=
            `🎁 Bonus        : <b>Rp${bonus.toLocaleString('id-ID')} (${percent}%)</b>\n` +
        ...
        msgUser +=
          `💼 Saldo kamu sekarang: <b>${saldoNow.toLocaleString(
            'id-ID'
          )}</b>`;

        try {
          await bot.telegram.sendMessage(chatId, msgUser, {
            parse_mode: 'HTML',
          });
        } catch (e) {
          logger.error(
            '❌ Gagal kirim notif topup QRIS ke user:',
            e.message
          );
        }
                        ...
                        if (GROUP_ID && NOTIF_TOPUP_GROUP) {
                          ...
                          if (chatInfo.username) {
                            userLabel = chatInfo.username; // tanpa @
                          } else if (chatInfo.first_name) {
                            userLabel = chatInfo.first_name;
                          } else {
                            userLabel = String(chatId); // fallback ke ID
                          }

                          const msgGroup =
                            '<blockquote>\n' +
                            '━━━━━━━━━━━━━━━━━━━━\n' +
                            '<b>💰 TOPUP SALDO (QRIS)</b>\n' +
                            '━━━━━━━━━━━━━━━━━━━━\n' +
                          ...
                          try {
                            ...
                            logger.error(
                              '❌ Gagal kirim notif topup QRIS ke grup:',
                              e.message
                            );
                          }
                        }
            ...
            logger.error('❌ Error di loop cek QRIS otomatis:', e);
          }
        }
      }
    );
  ...
  logger.info(
    `Scheduler cek QRIS otomatis aktif (interval=${INTERVAL_MS} ms, timeout=${EXPIRE_AFTER_MIN} menit).`
  );
}
// ===== END SECTION: PAYMENT - WORKER AUTO-TOPUP QRIS ========================

// Jalankan scheduler QRIS langsung saat start
startQrisAutoTopupChecker();

db.run(`
  CREATE INDEX IF NOT EXISTS idx_qris_payments_status_created
  ON qris_payments(status, created_at)
`);
...
bot.command('cekqris', async (ctx) => {
  ...
  try {
    ...
    try {
      const apiRes = await checkQrisInvoiceStatus(row.invoice_id, row.amount);
      if (apiRes) {
...
// ============================================================================
// SECTION: PAYMENT - QRIS AUTO TOPUP (ORDERKUOTA WRAPPER)
// - createQrisInvoice      : bikin QR dinamis + simpan ke file
// - checkQrisInvoiceStatus : cek status pembayaran ke OrderKuota
// ============================================================================

// Generate QRIS dinamis + simpan ke file sementara
async function createQrisInvoice(amount, note = '') {
  if (!ORDERKUOTA_BASE_QR || !ORDERKUOTA_AUTH_USERNAME || !ORDERKUOTA_AUTH_TOKEN) {
    throw new Error('Config ORDERKUOTA_BASE_QR / ORDERKUOTA_AUTH_USERNAME / ORDERKUOTA_AUTH_TOKEN belum di-set di .vars.json');
  }
  ...
  const { qrisGen } = getQrisInstances();

  // invoice_id akan kita pakai sebagai "reference" saat cek status
  const invoiceId = 'INV' + Date.now() + Math.floor(Math.random() * 9999);

  try {
    // 1) bikin QR string & gambar QR (buffer) pakai autoft-qris
    const qrString = qrisGen.generateQrString(nominal);
    const qrBuffer = await qrisGen.generateQRWithLogo(qrString); // QR dengan logo & tema

    // 2) simpan ke file sementara biar bisa dikirim ke Telegram
    const dir = path.join(__dirname, 'tmp_qris');
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, { recursive: true });
    }
    const fileName = `qris_${invoiceId}.png`;
    const filePath = path.join(dir, fileName);
    fs.writeFileSync(filePath, qrBuffer);

    // 3) kembalikan object invoice yang sudah dipakai handler /qris_topup
    return {
      invoice_id: invoiceId,
      amount: nominal,
      qris_image_path: filePath,
      qris_text: qrString,
      note,
    };
  } catch (e) {
    logger.error('❌ Gagal generate QRIS dinamis:', e);
    throw new Error('Gagal generate QRIS otomatis. Coba lagi beberapa saat.');
  }
}

// Cek status pembayaran ke OrderKuota
async function checkQrisInvoiceStatus(invoiceId, amount) {
  if (!ORDERKUOTA_AUTH_USERNAME || !ORDERKUOTA_AUTH_TOKEN) {
    throw new Error('Config ORDERKUOTA belum lengkap');
  }

  const nominal = Number(amount);
  if (!invoiceId || !Number.isFinite(nominal) || nominal <= 0) {
    throw new Error('Parameter cek QRIS tidak valid.');
  }

  const { qrisPaymentChecker } = getQrisInstances();

  try {
    const result = await qrisPaymentChecker.checkPaymentStatus(invoiceId, nominal);
    logger.info('QRIS checkPaymentStatus result:', {
      invoiceId,
      nominal,
      raw: result,
    });
    ...
    if (status === 'PAID') {
      const paidAt =
        result.data.paid_at
          ? new Date(result.data.paid_at).getTime() || Date.now()
          : Date.now();
    ...
    return {
  status: status || 'PENDING',   // jadi kelihatan misal 'SUCCESS' / 'SETTLED'
  paid_at: null,
  raw: result.data,
};
  } catch (e) {
    logger.error(`❌ Error cek status QRIS (invoice_id=${invoiceId}):`, e);
    return { status: 'PENDING', paid_at: null, error: e.message || String(e) };
  }
}
...
// === CEK DULU: APAKAH MASIH ADA INVOICE PENDING UNTUK USER INI? ===
  try {
    ...
    try {
      ...
      // 1. Buat invoice QRIS dengan NOMINAL BILLING (base + 3 digit acak)
      const invoice = await createQrisInvoice(
        billedAmount,
        `Topup saldo user ${userId} (base=${baseAmount}, random=${randomSuffix})`
      );
    ...
    `INSERT INTO qris_payments (
     ...
     VALUES (?, ?, ?, ?, ?, 'pending', ?)`,
    [userId, invoice.invoice_id, billedAmount, baseAmount, randomSuffix, now],
    (err) => (err ? reject(err) : resolve())
  );
});
      ...
      let caption =
        '💳 <b>Topup Saldo Otomatis (QRIS)</b>\n\n' +
        `Nominal yang kamu pilih : <b>Rp${baseAmount.toLocaleString('id-ID')}</b>\n` +
        (randomSuffix > 0
          ? `Nominal yang harus dibayar : <b>Rp${billedAmount.toLocaleString(
              'id-ID'
            )}</b>\n` +
            `(<i>Sudah termasuk kode unik ${randomSuffix.toString().padStart(
              3,
              '0'
            )}</i>)\n`
          : `Nominal yang harus dibayar : <b>Rp${billedAmount.toLocaleString(
			...
			'Silakan bayar sesuai <b>nominal yang harus dibayar</b>.\n' +
			'⏰ <b>QRIS ini hanya berlaku 5 menit.</b>\n' +
		    'Saldo akan masuk otomatis setelah pembayaran terdeteksi.';
...
// ============================================================================
// SECTION: PAYMENT - MANUAL TOPUP ENGINE (QRIS STATIS + MUTASI)
// - generateRandomNumber              : bikin kode unik nominal
// - processDeposit                    : kirim QRIS statis + catat pending_deposits
// - checkQRISStatus                   : cek mutasi (OrderKuota / bank) untuk deposit pending
// - getUserBalance                    : helper ambil saldo user
// - sendPaymentSuccessNotification    : notif ke user saat saldo masuk
// - processMatchingPayment            : update saldo + transaksi jika mutasi cocok
// ============================================================================
/*
...
async function processDeposit(ctx, amount) {
  ...
  // Tambah kode unik supaya bisa dicocokkan dari mutasi
  ...
  try {
    ...
    if (!fs.existsSync(qrisPath)) {
      throw new Error('File qris.jpg tidak ditemukan di folder bot');
    }

    const qrBuffer = fs.readFileSync(qrisPath);

    const caption =
      `?? *Detail Pembayaran:*\n\n` +
      `?? Jumlah yang harus dibayar: *Rp ${finalAmount}*\n` +
      `- Nominal Top Up : Rp ${amount}\n` +
      `- Admin Fee      : Rp ${adminFee}\n\n` +
      `?? *Penting:* Mohon transfer sesuai nominal di atas (hingga 3 digit terakhir).\n` +
    ...
    // Hapus state input nominal
    delete global.depositState[userId];

  } catch (error) {
    logger.error('? Kesalahan saat memproses deposit:', error);
    await ctx.editMessageText(
      '? *GAGAL! Terjadi kesalahan saat memproses pembayaran. Silakan coba lagi nanti.*',
      { parse_mode: 'Markdown' }
    );
    delete global.depositState[userId];
...
async function checkQRISStatus() {
  try {
    ...
    // Kalau tidak ada pending, tidak usah call API
    if (entries.length === 0) return;

    let transaksiList = [];

    // =============== AMBIL MUTASI SEKALI UNTUK SEMUA DEPOSIT PENDING ===============
    try {
      const data = buildPayload(); // dari api-cekpayment-orkut.js
      const resultcek = await axios.post(API_URL, data, { headers, timeout: 10000 });
      const resp = resultcek.data;

      // MODE LAMA: kalau suatu saat API balas teks
      if (typeof resp === 'string') {
        ...
        for (const block of blocks) {
          ...
          if (kreditMatch) {
            const nominal = parseInt(kreditMatch[1].replace(/[^\d]/g, ''), 10) || 0;
            transaksiList.push({
              kredit: nominal,
              amount: nominal,
              tanggal: tanggalMatch ? tanggalMatch[1].trim() : '',
              brand: brandMatch ? brandMatch[1].trim() : '',
              reference_id: tanggalMatch ? tanggalMatch[1].trim() : `text-${nominal}-${Date.now()}`
            });
          }
        }
      }
      // MODE BARU: JSON dari OrderKuota
      else if (resp && typeof resp === 'object') {
        if (!resp.success || !resp.qris_history || !resp.qris_history.success) {
          logger.warn('Respon OrderKuota tidak sukses:', JSON.stringify(resp));
          transaksiList = [];
        } else {
          const results = resp.qris_history.results || [];
          for (const item of results) {
            let rawNominal = item.kredit ?? item.jumlah ?? item.amount ?? item.nominal ?? 0;
            let nominal = 0;

            if (typeof rawNominal === 'string') {
              nominal = parseInt(rawNominal.replace(/[^\d]/g, ''), 10) || 0;
            } else {
              nominal = Number(rawNominal) || 0;
            }
        ...
        logger.warn('Format respon tidak dikenali dari API QRIS:', resp);
      }

      logger.info(`QRIS: total mutasi yang dibaca: ${transaksiList.length}`);
    } catch (err) {
      logger.error('Error saat mengambil mutasi QRIS:', err);
      return;
    }
    // =======================================================================
    ...
    for (const [uniqueCode, deposit] of entries) {
      ...
      if (!Array.isArray(transaksiList) || transaksiList.length === 0) continue;

      const expectedAmount = Math.round(Number(deposit.amount || 0));
      if (!expectedAmount) continue;

      // Cari mutasi dengan nominal sama persis (pakai kode unik)
      const matched = transaksiList.find((t) => t.amount === expectedAmount);

      if (matched) {
        try {
          const success = await processMatchingPayment(deposit, matched, uniqueCode);
          if (success) {
            logger.info(`Payment processed successfully for ${uniqueCode} (Rp ${expectedAmount})`);
            // Kalau processMatchingPayment belum hapus, hapus di sini juga
            delete global.pendingDeposits[uniqueCode];
            db.run(
              'DELETE FROM pending_deposits WHERE unique_code = ?',
              [uniqueCode],
          ...
          logger.error(`Error processing matching payment for ${uniqueCode}:`, err);
        }
      }
    }
    // =======================================================================
...
async function updateUserBalance(userId, amount) {
  ...
  return new Promise((resolve, reject) => {
    if (!amt || amt <= 0) {
      logger.warn('⚠️ updateUserBalance dipanggil dengan amount tidak valid:', amount);
      return reject(new Error('Jumlah saldo tidak valid'));
    }
    ...
    db.run(
      ...
      function (err) {
        ...
        // Setelah update, ambil saldo terbaru
        db.get(
          'SELECT saldo FROM users WHERE user_id = ?',
          [userId],
          async (err2, row) => {
            if (err2 || !row) {
              logger.error(
                '⚠️ Gagal mengambil saldo setelah update:',
                err2 ? err2.message : 'row kosong'
              );

              // Tetap catat transaksi walaupun saldo terbaru tidak kebaca
              try {
                recordSaldoTransaction(
                  Number(userId),
                  amt,
                ...
                logger.error('⚠️ Gagal mencatat transaksi saldo (addsaldo_panel):', e.message);
              }

              return resolve();
            }

            const newSaldo = Number(row.saldo || 0);
  ...
  .catch((e) => {
    logger.error(
      ...
      e.message
            ...
            // 📨 Notif ke grup (kalau GROUP_ID diset)
            try {
              if (typeof GROUP_ID !== 'undefined' && GROUP_ID) {
                ...
                const notifTopup =
				  ...
				  '<code>\n' + // <-- MULAI BLOK MONOSPACE
                  'User   : ' + userId + '\n' +
                  'Topup  : Rp ' + amt.toLocaleString() + '\n' +
                  'Saldo  : Rp ' + newSaldo.toLocaleString() + '\n' +
                  'Status : SUCCESS\n' +
                  'Tanggal: ' + waktu + '\n' +
				  '</code>\n' + // <-- AKHIR BLOK MONOSPACE
                  '━━━━━━━━━━━━━━━━━━━━\n' +
              ...
              logger.error(
                '⚠️ Gagal mengirim notif topup ke grup (addsaldo_panel):',
                e.message
              );
            }
...
async function sendPaymentSuccessNotification(userId, deposit, currentBalance, bonusInfo) {
  try {
    const baseAmount = Number(deposit.originalAmount) || 0;
    const adminFee = Number(deposit.amount || 0) - baseAmount;

    let text =
      '✅ *Pembayaran Berhasil!*\n\n' +
      `💰 Jumlah Deposit : Rp ${baseAmount.toLocaleString('id-ID')}\n` +
      `💰 Biaya Admin    : Rp ${adminFee.toLocaleString('id-ID')}\n` +
      `💰 Total Bayar    : Rp ${Number(deposit.amount || 0).toLocaleString('id-ID')}\n`;

    if (bonusInfo && bonusInfo.bonus > 0) {
      const totalCredit = baseAmount + bonusInfo.bonus;
      text +=
        `\n🎁 Bonus Saldo    : Rp ${bonusInfo.bonus.toLocaleString('id-ID')} (${bonusInfo.percent}%)\n` +
        `💳 Saldo Masuk    : Rp ${totalCredit.toLocaleString('id-ID')}\n`;
    } else {
      text += `\n💳 Saldo Masuk    : Rp ${baseAmount.toLocaleString('id-ID')}\n`;
    }

    text += `\n💼 Saldo Sekarang : Rp ${Number(currentBalance || 0).toLocaleString('id-ID')}`;
...
async function processMatchingPayment(deposit, matchingTransaction, uniqueCode) {
  ...
  // Dasar bonus = jumlah deposit asli (tanpa admin fee)
  const baseAmount = Number(deposit.originalAmount) || 0;

  // Hitung bonus pakai tier yang sudah kamu set di calculateTopupBonus
  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      const refId = matchingTransaction.reference_id || uniqueCode;

      // Cek apakah transaksi ini sudah pernah diproses (berdasarkan reference_id)
      db.get(
        ...
        (err, row) => {
          ...
          // Update saldo user: saldo + (deposit + bonus)
          db.run(
            'UPDATE users SET saldo = saldo + ? WHERE user_id = ?',
            ...
            function (err) {
              ...
              db.run(
                'INSERT INTO transactions (user_id, amount, type, reference_id, timestamp) VALUES (?, ?, ?, ?, ?)',
                [deposit.userId, totalCredit, 'deposit', refId, Date.now()],
                (err) => {
                  if (err) {
                    db.run('ROLLBACK');
                  ...
                  db.get(
                    ...
                    async (err, user) => {
                      ...
                      // Kirim notif ke user (pakai bonusInfo)
                      ...
                      if (notificationSent) {
                        // Notifikasi ke grup (log top up)
                        try {
                          let userInfo;
                          try {
                            userInfo = await bot.telegram.getChat(
                              deposit ? deposit.userId : (ctx ? ctx.from.id : '')
                            );
                          } catch (e) {
                            userInfo = {};
                          }
                          ...
                          if (userInfo.username) {
                            userLabel = userInfo.username; // tanpa @
                          } else if (userInfo.first_name) {
                            userLabel = userInfo.first_name;
                          } else {
                            userLabel = String(deposit ? deposit.userId : (ctx ? ctx.from.id : ''));
                          }

                          const baseStr = baseAmount.toLocaleString('id-ID');
                          const bonusStr = bonus.toLocaleString('id-ID');
                          const totalCreditStr = totalCredit.toLocaleString('id-ID');
                          const saldoNowStr = Number(user.saldo || 0).toLocaleString('id-ID');

                          let groupMsg =
                            `<blockquote>
✅ <b>Top Up Berhasil</b>
👤 User           : ${userLabel}
💰 Nominal Deposit: <b>Rp ${baseStr}</b>
`;

                          if (bonus > 0) {
                            groupMsg +=
                              `🎁 Bonus Saldo    : <b>Rp ${bonusStr} (${percent}%)</b>
💳 Saldo Masuk    : <b>Rp ${totalCreditStr}</b>
`;
                          } else {
                            groupMsg += `💳 Saldo Masuk    : <b>Rp ${totalCreditStr}</b>
`;
                          }

                          groupMsg +=
                            `🏦 Saldo Sekarang : <b>Rp ${saldoNowStr}</b>
🕒 Waktu          : ${new Date().toLocaleString('id-ID', { timeZone: TIME_ZONE })}
</blockquote>`;
...
// ===== END SECTION: PAYMENT - MANUAL TOPUP ENGINE (QRIS STATIS + MUTASI) ===
